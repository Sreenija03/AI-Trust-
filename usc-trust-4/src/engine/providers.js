...
// ---------------------------------------------------------------------------
// Live model discovery — query a provider's list endpoint with the user's key
// and return the model IDs it actually supports. Survives provider renames.
// Returns [{ apiModel, label }]. Filters to models usable for generation.
// ---------------------------------------------------------------------------
export async function listModels(provider, keys, { signal } = {}) {
  if (provider === "google") {
    if (!keys.google) throw new Error("Add your Google key first.");
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${keys.google}&pageSize=200`, { signal });
    if (!res.ok) throw new Error(`Google ListModels ${res.status}: ${(await res.text()).slice(0, 160)}`);
    const data = await res.json();
    return (data.models || [])
      // only models that support generateContent (i.e. usable as chat models)
      .filter((m) => (m.supportedGenerationMethods || []).includes("generateContent"))
      .map((m) => {
        const id = (m.name || "").replace(/^models\//, "");
        return { apiModel: id, label: m.displayName ? `${m.displayName} (${id})` : id };
      })
      .filter((m) => m.apiModel && !/embedding|aqa|imagen/i.test(m.apiModel));
  }
  if (provider === "openai" || provider === "openai_compat") {
    const key = provider === "openai" ? keys.openai : keys.openai_compat;
    const base = provider === "openai" ? "https://api.openai.com/v1" : (keys.openai_compat_base || "");
    if (!key) throw new Error("Add your key first.");
    const res = await fetch(`${base}/models`, { signal, headers: { authorization: `Bearer ${key}` } });
    if (!res.ok) throw new Error(`ListModels ${res.status}: ${(await res.text()).slice(0, 160)}`);
    const data = await res.json();
    return (data.data || [])
      .map((m) => ({ apiModel: m.id, label: m.id }))
      .filter((m) => /gpt|o\d|chat|llama|mistral|qwen|gemma|claude/i.test(m.apiModel) && !/embed|whisper|tts|dall|image|moderation|audio|realtime/i.test(m.apiModel))
      .sort((a, b) => a.apiModel.localeCompare(b.apiModel));
  }
  if (provider === "anthropic") {
    if (!keys.anthropic) throw new Error("Add your Anthropic key first.");
    const res = await fetch("https://api.anthropic.com/v1/models", {
      signal,
      headers: { "x-api-key": keys.anthropic, "anthropic-version": "2023-06-01", "anthropic-dangerous-direct-browser-access": "true" },
    });
    if (!res.ok) throw new Error(`Anthropic ListModels ${res.status}: ${(await res.text()).slice(0, 160)}`);
    const data = await res.json();
    return (data.data || []).map((m) => ({ apiModel: m.id, label: m.display_name ? `${m.display_name} (${m.id})` : m.id }));
  }
  if (provider === "xai") {
    if (!keys.xai) throw new Error("Add your xAI key first.");
    // xAI exposes an OpenAI-compatible /models endpoint.
    const res = await fetch("https://api.x.ai/v1/models", { signal, headers: { authorization: `Bearer ${keys.xai}` } });
    if (!res.ok) throw new Error(`xAI ListModels ${res.status}: ${(await res.text()).slice(0, 160)}`);
    const data = await res.json();
    return (data.data || [])
      .map((m) => ({ apiModel: m.id, label: m.id }))
      .filter((m) => !/embed|image|vision-embed/i.test(m.apiModel))
      .sort((a, b) => a.apiModel.localeCompare(b.apiModel));
  }
  throw new Error(`Model listing not supported for ${provider}.`);
}
