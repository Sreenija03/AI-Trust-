...
export default function Leaderboard() {
  const { liveResults, removeResult, clearResults } = useAppState();
      ...
      <div className="container">
        ...
        <div className="row wrap" style={{ marginBottom: 16, gap: 10 }}>
          ...
          {liveResults.length > 0 && (
            <button className="btn btn-ghost btn-sm" onClick={() => { if (confirm("Remove all your saved evaluation runs from this browser? The two reference models stay.")) clearResults(); }}>
              Clear saved runs ({liveResults.length})
            </button>
          )}
        ...
        {liveResults.length > 0 && (
          <p className="mono" style={{ fontSize: 11, color: "var(--text-faint)", margin: "0 0 16px" }}>
            Your {liveResults.length} measured run{liveResults.length > 1 ? "s are" : " is"} saved in this browser and will stay on the leaderboard when you reopen the site.
          </p>
        )}
