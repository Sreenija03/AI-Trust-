...
import { runWithVariance } from "../engine/variance.js";
import { MODEL_REGISTRY, isModelRunnable, generateResponse, JUDGE_REGISTRY, DEFAULT_JUDGE, familyOf, readyProviders, PROVIDER_FAMILY } from "../engine/providers.js";
import { download, varianceToCSV } from "../engine/exporters.js";
import { useAppState } from "../state/AppState.jsx";
import ApiKeyPanel from "../components/ApiKeyPanel.jsx";
import BiasHarness from "../components/BiasHarness.jsx";

// Merge static registry with models discovered live via each provider's API.
...
export default function Runner({ go }) {
  const { keys, addResult, discoveredModels } = useAppState();
      ...
      <div className="container">
        <div className="section-head">
          ...
          <p>
            ...
            against the framework's rubric. Every score is measured live and populates the leaderboard.
          </p>
        </div>
        ...
        <JudgePanel judgeLabel={judgeLabel} setJudgeLabel={setJudgeLabel} judgeEntry={judgeEntry}
          ready={ready} selfFamily={selfFamily} modelEntry={modelEntry} judges={judges} />
        ...
        <div className="row wrap" style={{ gap: 10, margin: "20px 0" }}>
          <div className="row" style={{ gap: 6 }}>
            ...
            <button className={`btn btn-sm ${mode === "bias" ? "btn-primary" : "btn-ghost"}`} onClick={() => setMode("bias")}>Multi-judge bias</button>
          </div>
        </div>

        {mode === "full" && <FullRunner modelEntry={modelEntry} modelLabel={modelLabel} setModelLabel={setModelLabel} keys={keys} runnable={runnable} addResult={addResult} go={go} judgeEntry={judgeEntry} selfFamily={selfFamily} models={models} />}
        {mode === "single" && <SingleRunner modelEntry={modelEntry} modelLabel={modelLabel} setModelLabel={setModelLabel} keys={keys} runnable={runnable} judgeEntry={judgeEntry} selfFamily={selfFamily} models={models} />}
        {mode === "bias" && <BiasHarness models={models} judges={judges} keys={keys} ready={ready} />}
...
function JudgePanel({ judgeLabel, setJudgeLabel, judgeEntry, ready, selfFamily, modelEntry, judges }) {
  return (
...
function ModelPicker({ modelLabel, setModelLabel, keys, models }) {
  return (
    <div>
      ...
      <select className="select" value={modelLabel} onChange={(e) => setModelLabel(e.target.value)}>
        {models.map((m) => {
          const ok = Boolean(keys[m.provider]);
...
// ---- Full battery runner ------------------------------------------------
function FullRunner({ modelEntry, modelLabel, setModelLabel, keys, runnable, addResult, go, judgeEntry, selfFamily, models }) {
  ...
  async function start() {
    setError(""); setResult(null); setVariance(null); setLiveRows([]);
    if (!runnable) {
      setError(`Add both the judge key and the ${modelEntry.provider} key to run ${modelLabel}.`);
      return;
    }
    setRunning(true);
    ...
    try {
      if (runs > 1) {
        ...
        const v = await runWithVariance(modelEntry, battery, keys, {
          runs, judgeEntry, signal: ctrl.signal,
          onProgress: ({ run, runs: total, inner }) => {
            setProgress({ ...inner, runLabel: `run ${run}/${total}` });
            if (inner?.result) setLiveRows((rows) => [...rows.slice(-92), inner.result]);
          },
        });
        setVariance(v);
        // Put the mean run on the leaderboard.
        const meanRecord = { ...v.records[v.records.length - 1], overall: v.overall.mean, model: modelEntry.label };
        meanRecord.pillars = Object.fromEntries(Object.entries(v.pillars).map(([k, s]) => [k, { score: s.mean, passed: meanRecord.pillars[k]?.passed || 0, total: meanRecord.pillars[k]?.total || 0 }]));
        meanRecord.variance = { overall: v.overall, pillars: v.pillars, runs: v.runs };
        setResult(meanRecord);
        addResult(meanRecord);
      } else {
  ...
  return (
    <div className="runner-grid">
      <div className="card card-pad">
        <ModelPicker modelLabel={modelLabel} setModelLabel={setModelLabel} keys={keys} models={models} />
        ...
        <div className="row wrap" style={{ gap: 8, marginTop: 12, alignItems: "center" }}>
          <span className="mono" style={{ fontSize: 11, color: "var(--text-muted)" }}>RUNS</span>
          {[1, 3, 5].map((n) => (
            <button key={n} className={`btn btn-sm ${runs === n ? "btn-primary" : "btn-ghost"}`} onClick={() => setRuns(n)}>
              {n === 1 ? "Single run" : `${n}× (± CI)`}
            </button>
          ))}
          {runs > 1 && <span className="mono" style={{ fontSize: 10.5, color: "var(--text-faint)" }}>~{battery.length * runs * 2} calls · gives confidence intervals</span>}
        </div>

        {!running ? (
          <button className="btn btn-primary" style={{ marginTop: 14, width: "100%", justifyContent: "center" }} onClick={start} disabled={!runnable}>
            {runs > 1 ? `Run ${runs}× with confidence intervals` : `Run full evaluation (${battery.length} tests)`}
          </button>
        ) : (
          <button className="btn btn-ghost" style={{ marginTop: 14, width: "100%", justifyContent: "center" }} onClick={stop}>
            Stop {progress?.runLabel ? `(${progress.runLabel})` : ""}
          </button>
        )}
        ...
        {variance && (
          <div className="card card-pad" style={{ marginTop: 18 }}>
            <div className="row" style={{ justifyContent: "space-between", marginBottom: 8 }}>
              <h4 style={{ fontSize: 15, margin: 0 }}>Mean ± 95% CI over {variance.runs} runs</h4>
              <button className="btn btn-ghost btn-sm" onClick={() => download(`variance-${variance.model}.csv`, varianceToCSV(variance), "text/csv")}>↓ CSV</button>
            </div>
            <div className="var-overall mono">
              Overall: <strong>{variance.overall.mean}</strong> ± {variance.overall.ci}
              <span style={{ color: "var(--text-faint)" }}> (95% CI {variance.overall.lo}–{variance.overall.hi}; runs {variance.perRunOveralls.join(", ")})</span>
            </div>
            <table className="var-table">
              <thead><tr><th>Pillar</th><th>Mean</th><th>± CI</th><th>SD</th><th>Range</th></tr></thead>
              <tbody>
                {Object.entries(variance.pillars).map(([pid, s]) => (
                  <tr key={pid}>
                    <td>{PILLAR_BY_ID[pid]?.short || pid}</td>
                    <td className="mono" style={{ fontWeight: 700 }}>{s.mean}</td>
                    <td className="mono">±{s.ci}</td>
                    <td className="mono" style={{ color: "var(--text-muted)" }}>{s.sd}</td>
                    <td className="mono" style={{ color: "var(--text-faint)" }}>{s.min}–{s.max}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function SingleRunner({ modelEntry, modelLabel, setModelLabel, keys, runnable, judgeEntry, selfFamily, models }) {
...
.var-overall { font-size: 13px; margin-bottom: 10px; }
.var-table { width: 100%; border-collapse: collapse; font-size: 12.5px; }
.var-table th { text-align: left; font-size: 10px; color: var(--text-muted); letter-spacing: 0.05em; padding: 4px 8px; border-bottom: 1px solid var(--line); }
.var-table td { padding: 5px 8px; border-bottom: 1px solid var(--line); }
