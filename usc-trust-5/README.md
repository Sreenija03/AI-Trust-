## Toward publishable results (validated datasets, variance, stats export)

Three additions move the platform from demonstration toward analyzable data:

1. **Validated benchmark datasets** (`src/data/datasets.js`, `src/pages/Datasets.jsx`). A "Datasets" page
   runs the model against real, peer-reviewed benchmarks with their own published criteria, replacing
   reconstructed prompts: **XSTest** (Röttger 2024, over-refusal), **CrowS-Pairs** (Nangia 2020, stereotype
   bias), **TruthfulQA** (Lin 2022, truthfulness). These are disclosed curated samples of the full sets,
   which can be loaded in full from the cited repos.
2. **Multi-run variance + 95% confidence intervals** (`src/engine/variance.js`). The runner can run 3× or 5×
   and reports mean ± CI per pillar and overall, so results carry error bars instead of single points.
3. **Paper-ready CSV exports** (`varianceToCSV`, `biasToCSV`, `agreementToCSV` in `exporters.js`) for dropping
   results tables straight into an analysis or appendix.

What still requires human/lab work (cannot be automated here): collecting real human ratings for the
agreement study, running the studies at scale with funded keys, swapping in full (not sampled) item banks,
and the lab's decision on venue/authorship.

## Human review + agreement (LLM + human, per professor direction)

A **Human Review** page lets a reviewer score the same responses the LLM judge scored, using the SAME
explicit criterion shown on screen, then computes human-vs-LLM agreement: raw agreement, **Cohen's kappa**
(with Landis-Koch interpretation), mean absolute difference, and a pass/fail confusion breakdown that
surfaces judge leniency (LLM-pass / human-fail). Export a CSV for offline raters, or score in-app. This is
the validation step that turns LLM-as-judge into a defensible LLM+human method, and produces the labeled
data a future small judge model would train on. Engine: `src/engine/agreement.js`; page:
`src/components/HumanReview.jsx`.

## Version 2 framework (Unified AI Trust Framework)

The site now includes a **Framework V2** page documenting the newer USC/Tumeryk "Unified AI Trust
Framework": 7 dimensions across 2 layers (Model Intrinsic Risk; Deployment & Supply-Chain Risk), ~26
sub-categories, each with its definition, evaluation method, real dataset/probe mapping, and a link back
to the Version-1 USC pillar where one exists. Live scoring still runs on Version 1; the V2 page honestly
marks each sub-category as scored-live-now, needs-a-dataset, needs-a-probe-harness, or roadmap, and
computes overall live coverage. Data: `src/data/frameworkV2.js`; page: `src/pages/FrameworkV2.jsx`.

## Explicit judge criteria (per professor direction)

Judge prompts now embed the EXACT per-sub-category criteria from the framework's Probe_Progress sheet
(a binary 0 = pass / 1 = fail detector with written definitions), via `src/engine/judgeCriteria.js`. The
judge is told precisely what to check rather than inferring it. This is the groundwork for the planned
human-review layer (compare human vs LLM scores) and a future small fine-tuned judge model.
...
## Picking models that actually exist (model discovery)

Providers rename and retire models often (e.g. Gemini 1.5 → 2.x), which causes "model not found"
(404) errors against hardcoded names. To avoid this, each provider key has a **Fetch models** button:
it calls that provider's live model-list endpoint with your key and loads the exact models your key
can use into the target and judge dropdowns. Click it after entering a key; if a provider renames
things later, just hit Refresh. Supported for OpenAI, Anthropic, Google, and OpenAI-compatible
endpoints.

## Multi-judge bias harness (research mode)

A third runner mode that quantifies judge bias instead of just scoring a model. It generates each response
**once**, then has 2–3 judges (ideally from different companies) score the identical text. It then reports:

- **Self-family inflation** — how many points the target's own-company judge gives versus the mean of
  independent judges, on the same responses. This is the headline metric: a positive delta is the
  self-preference effect documented in the LLM-as-judge literature.
- **Per-judge mean scores** — does one judge systematically score higher across all tests?
- **Inter-judge agreement** — pairwise Pearson r and mean absolute difference between every judge pair, plus
  the overall mean score spread. The signature of bias is a judge that agrees with the others *except* when it
  shares the target's family.

Engine: `src/engine/biasHarness.js` (with `judgeResponseOnly` in `scorer.js`). Pick a target, select judges,
choose Sample (~16 tests) or Full (93), and run. Results export to JSON with the full per-test, per-judge
breakdown — suitable as evidence in a write-up. Needs keys for each judge's provider.
